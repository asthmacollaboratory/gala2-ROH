...
ConcatenateResults = function(input.prefix, output.path, input.suffix = "ROH.R.out.results", sort.result = FALSE) {
    ...
    # sort frame by p-value, if desired
    if (sort.result) {
...
CreateDiagnosticPlots = function(results.filepath, manhattan.plot.filepath, qq.plot.filepath,
    manhattan.plot.title = "Manhattan plot", threshold = 5e-8, highlight.SNPs = NULL,
    manhattan.ylims = c(0,8), color = c("black", "blue"), significance.threshold = 5e-8,
    suggestive.threshold = 1e-7, qq.plot.title = "QQ Plot", qq.plot.subtitle = NULL,
    qq.xlim = NULL, qq.ylim = NULL, plot.width = 7, plot.height = 7, plot.units = "in") {
...
ComputeSignificanceThreshold = function(input.file) {
    ...
    # effectiveSize() adjusts for autocorrelation (correlation between adjacent p-values) and calculates the number of independent tests
    # use -log10(p) instead of p (e.g. 6 vs. 10^-6) when calculating effectiveSize to guard against underflow 
    # Output of effectiveSize() is :
    # var 1
    # Number
    # only need the threshold (Number), so as.numeric() fixes this
    total_eff = as.numeric(effectiveSize(-log10(copy.input$p)))
    ...
    # return significance thresholds in a list 
    return(list("n.indep.test" = total_eff, "bon.corr" = p.adj, "transformation" = transformation, "sugg.thresh" = suggestive))
}

RohLancPlot = function(input.pheno, phenotype, input.dosage, input.ancestry, probe.min, probe.max, colorblind.friendly = FALSE,
    size = 0.05, sig.probe, roh.title = "ROH Plot", roh.subtitle = NULL, ancestry.title = "Local Ancestry Plot",
    ancestry.subtitle = NULL, sum.title = "Local Ancestry Summary Plot", sum.subtitle = NULL, roh.lanc.title = "ROH Local Ancestry",
    ...
    #                 Assuming the default value "png" of `plot.type`, the following plot suffixes are generated: 
    ...
    #     plot.units: the units of the plot window, e.g. "in" or "cm" 
    #         Default: "in" (inches) 
    #     plot.type: the file type, e.g. "png", for the plots to be generated 
    #         Default: "png"
    ...
    RohPlot(input.df = roh.final.merge,
        ...
        output.path = paste(output.pfx, "roh", plot.type),
        plot.width = plot.width,
        plot.height = plot.height,
        plot.units = plot.units
    )
